...
def check_valid_tensor(
    input_tensor: Optional[torch.Tensor], name: str = "tensor"
) -> None:
    """
...
def normalize_camera_extrinsics_and_points_batch(
    ...
    if scale_by_points:
        ...
        new_extrinsics[:, :, :3, 3] = new_extrinsics[:, :, :3, 3] / avg_scale.view(
            -1, 1, 1
        )
    ...
    new_extrinsics = check_and_fix_inf_nan(
        new_extrinsics, "new_extrinsics", hard_max=None
    )
    new_cam_points = check_and_fix_inf_nan(
        new_cam_points, "new_cam_points", hard_max=None
    )
    new_world_points = check_and_fix_inf_nan(
        new_world_points, "new_world_points", hard_max=None
    )
