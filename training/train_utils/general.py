...
def copy_data_to_device(data, device: torch.device, *args: Any, **kwargs: Any):
    ...
    elif isinstance(data, Mapping) and not is_dataclass(
        data
    ):  # handing FrameData-like things
        return type(data)(
            {
                k: copy_data_to_device(v, device, *args, **kwargs)
                for k, v in data.items()
...
def safe_makedirs(path: str):
    if not path:
        logging.warning(
            "safe_makedirs called with an empty path. No operation performed."
        )
        return False
    ...
    try:
        ...
        logging.error(
            f"An unexpected error occurred while creating directory '{path}'. Reason: {e}"
        )
        raise
...
class AverageMeter:
    ...
    def __init__(
        self, name: str, device: Optional[torch.device] = None, fmt: str = ":f"
    ):
        self.name = name
        self.fmt = fmt
        self.device = device
