...
def build_tracks_by_depth(
    ...
    if (query_point_masks).sum() > 0:
        ...
        for shift in [(0, 0), (1, 0), (0, 1), (1, 1)]:
            ...
            if depth_inside_flag is None:
                ...
                depth_inside_flag = torch.logical_or(
                    depth_inside_flag, cur_depth_inside_flag
                )
    ...
    delta_us = (
        torch.rand(size=(B, sampled_neg_track_num), device=world_points.device)
        * perb_range[0]
    )
    delta_vs = (
        torch.rand(size=(B, sampled_neg_track_num), device=world_points.device)
    ...
    negative_sampson_distances = track_epipolar_check(
        negative_tracks, extrinsics, intrinsics
    )
    ...
    final_pos_masks = torch.zeros(
        target_track_num, device=world_points.device, dtype=torch.bool
    )
    ...
    rand_indices = torch.randperm(
        negative_tracks.shape[1], device=negative_tracks.device
    )
...
def sample_positive_tracks(
    tracks, tracks_mask, track_num, half_top=True, seq_name=None
):
    # tracks: (B, T, 2)
    # tracks_mask: (B, T)
    # track_num: int
...
def track_epipolar_check(tracks, extrinsics, intrinsics, use_essential_mat=False):
    ...
    essential_mats = get_essential_matrix(
        extrinsics[0:1].expand(B - 1, -1, -1), extrinsics[1:]
    )
    ...
    if use_essential_mat:
        ...
        fundamental_mats = (
            K2.inverse().permute(0, 2, 1).matmul(essential_mats).matmul(K1.inverse())
        )
        sampson_distances = sampson_epipolar_distance(
...
def visualize_tracks_on_images(
    ...
    for b in range(B):
        ...
        print(
            f"[INFO] Saved color-by-XY track visualization for sample b={b} -> {out_path}"
        )
