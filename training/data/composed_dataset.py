...
class ComposedDataset(Dataset, ABC):
    ...
    def __getitem__(self, idx_tuple):
        ...
        ids = torch.from_numpy(
            batch["ids"]
        )  # Frame indices sampled from the original sequence
        ...
        if self.load_track:
            if batch["tracks"] is not None:
                ...
                track_vis_mask = torch.from_numpy(
                    np.stack(batch["track_masks"]).astype(bool)
                )
                ...
                if len(valid_indices) >= self.track_num:
                    ...
                    sampled_indices = valid_indices[torch.randperm(len(valid_indices))][
                        : self.track_num
                    ]
                else:
