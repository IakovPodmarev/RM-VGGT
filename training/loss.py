...
def camera_loss_single(pred_pose_enc, gt_pose_enc, loss_type="l1"):
    ...
    if loss_type == "l1":
        ...
        loss_T = (pred_pose_enc[..., :3] - gt_pose_enc[..., :3]).norm(
            dim=-1, keepdim=True
        )
...
def compute_depth_loss(
    ...
    **kwargs,
    ...
    gt_depth_mask = batch[
        "point_masks"
    ].clone()  # 3D points derived from depth map, so we use the same mask
...
def gradient_loss_multi_scale_wrapper(
    prediction, target, mask, scales=4, gradient_loss_fn=None, conf=None
):
    """
...
def normal_loss(
    prediction, target, mask, cos_eps=1e-8, conf=None, gamma=1.0, alpha=0.2
):
    """
